...
hw-new:
	...
	@cd ./vivado && cp -f ../$(DUT).tcl ./top.tcl
	@cd ./vivado && cp -f ../$(DUT).xdc ./top.xdc
	@cd ./vivado && vivado -mode tcl -source ../vivado_new.tcl


hw-com:
	@cd ./vivado && \rm -rf vivado.* .Xil *.os *.jou *.og  $(PRJ).cache $(PRJ).runs
	@cd ./vivado && vivado -mode tcl -project ./$(PRJ).xpr -source ../vivado_com.tcl
